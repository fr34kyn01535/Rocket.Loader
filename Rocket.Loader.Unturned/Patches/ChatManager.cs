...
namespace Rocket.RocketLoader.Unturned.Patches
{
    ...
    public class ChatManager : Patch
    {
        public override void Apply()
        {
            ...
            if (process != null)
            {
                process.Body.Instructions[11] = Instruction.Create(OpCodes.Call, RocketLoader.UnityAssemblyDefinition.MainModule.ImportReference(checkPermissions));
                process.Body.GetILProcessor().InsertBefore(process.Body.Instructions[11], Instruction.Create(OpCodes.Ldarg_1));
            }
