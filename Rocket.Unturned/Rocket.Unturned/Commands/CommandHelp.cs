    ...
    public class CommandHelp : IRocketCommand
    {
        ...
        public void Execute(UnturnedPlayer caller, string[] command)
        {
            if (command.Length == 0)
            {
                ...
                System.Console.ForegroundColor = ConsoleColor.White;
                Commander.Commands.Where(c => c is RocketCommandBase && ((RocketCommandBase)c).Command.GetType().Assembly == Assembly.GetExecutingAssembly()).OrderBy(c => c.commandName).All(c => { System.Console.WriteLine(c.commandName.ToLower().PadRight(20, ' ') + " " + c.commandInfo.ToLower()); return true; });
                ...
                foreach (IRocketPlugin plugin in RocketPluginManager.Plugins)
                {
                    ...
                    System.Console.ForegroundColor = ConsoleColor.White;
                    Commander.Commands.Where(c => c is RocketCommandBase && ((RocketCommandBase)c).Command.GetType().Assembly == plugin.GetType().Assembly).OrderBy(c => c.commandName).All(c => { System.Console.WriteLine(c.commandName.ToLower().PadRight(20, ' ') + " " + c.commandInfo.ToLower()); return true; });
                    System.Console.WriteLine();
                }
            }
            else
